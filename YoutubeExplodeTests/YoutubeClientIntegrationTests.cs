...
namespace YoutubeExplode.Tests
{
    ...
    public class YoutubeClientIntegrationTests
    {
        ...
        [TestMethod]
        public async Task GetVideoCaptionTrackAsync_Test()
        {
            var videoInfo = await _client.GetVideoInfoAsync("_QdPW8JrYzQ");
            var trackInfo = videoInfo.CaptionTracks.FirstOrDefault(c => c.Language == "en");
            var track = await _client.GetVideoCaptionTrackAsync(trackInfo);
            var caption = track.GetCaptionFor(TimeSpan.FromSeconds(40));
