...
namespace YoutubeExplode
{
    public partial class YoutubeClient
    {
        ...
        public async Task<IReadOnlyList<ClosedCaptionTrackInfo>> GetVideoClosedCaptionTrackInfosAsync(string videoId)
        {
            ...
            // Get closed caption track infos encoded in JSON
            var closedCaptionTrackInfosJson = playerResponseJson.SelectToken("..captionTracks");

            // Extract closed caption track infos
            var closedCaptionTrackInfos = new List<ClosedCaptionTrackInfo>();
            foreach (var closedCaptionTrackInfoJson in closedCaptionTrackInfosJson.EmptyIfNull())
            {
                // Get URL
                var url = closedCaptionTrackInfoJson.SelectToken("baseUrl").Value<string>();

                // Set format to the one we know how to deal with
                url = Url.SetQueryParameter(url, "format", "3");

                // Get language
                var languageCode = closedCaptionTrackInfoJson.SelectToken("languageCode").Value<string>();
                var languageName = closedCaptionTrackInfoJson.SelectToken("name.simpleText").Value<string>();
                var language = new Language(languageCode, languageName);

                // Get whether the track is autogenerated
                var isAutoGenerated = closedCaptionTrackInfoJson.SelectToken("vssId").Value<string>()
