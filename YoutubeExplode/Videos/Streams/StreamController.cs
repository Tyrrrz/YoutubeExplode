...
internal class StreamController : VideoController
{
    ...
    public async Task<PlayerSourceExtractor?> TryGetPlayerSourceAsync(CancellationToken cancellationToken = default)
    {
        var iframeContent = await SendHttpRequestAsync("https://www.youtube.com/iframe_api", cancellationToken);

        var version = Regex.Match(iframeContent, @"player\\?/([0-9a-fA-F]{8})\\?/").Groups[1].Value;
        if (string.IsNullOrWhiteSpace(version))
            return null;

        var source = await SendHttpRequestAsync($"https://www.youtube.com/s/player/{version}/player_ias.vflset/en_US/base.js", cancellationToken);
