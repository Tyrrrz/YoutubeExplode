...
namespace YoutubeExplode.Tests
{
    ...
    public class YoutubeClientIntegrationTests
    {
        ...
        public async Task GetPlaylistInfoAsync_LargeTruncated_Test()
        {
            ...
            // Video ids
            Assert.IsNotNull(playlistInfo.VideoIds);
            Assert.IsTrue(1000 >= playlistInfo.VideoIds.Count); // every page contains no more than 500 videos
        }
        ...
        public async Task GetPlaylistInfoAsync_VideoMix_Test()
        {
            ...
            Assert.IsTrue(string.IsNullOrEmpty(playlistInfo.Author)); // video mixes have no author
        ...
        public async Task GetPlaylistInfoAsync_ChannelMix_Test()
        {
            ...
            Assert.AreEqual("Лига Смеха", playlistInfo.Author);
            Assert.IsTrue(string.IsNullOrEmpty(playlistInfo.Description)); // channel mixes have no description
            Assert.IsTrue(0 <= playlistInfo.ViewCount);
            // Likes/dislikes not checked because I don't know where they come from

            // Video ids (not predictable because it's a mix)
            Assert.IsNotNull(playlistInfo.VideoIds);
            Assert.IsTrue(23 <= playlistInfo.VideoIds.Count);
        }
